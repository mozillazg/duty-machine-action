...
class FilterEngine extends events_1.EventEmitter {
    ...
    static fromLists(fetch, urls, config = {}, caching) {
        return this.fromCached(() => {
            const listsPromises = fetch_1.fetchLists(fetch, urls);
            const resourcesPromise = fetch_1.fetchResources(fetch);
    ...
    static parse(filters, options = {}) {
        ...
        return new this(Object.assign(Object.assign({}, lists_1.parseFilters(filters, config)), { config }));
    }
     ...
     */
    getSerializedSize() {
        let estimatedSize = data_view_1.sizeOfByte() + // engine version
        ...
        for (const [name, checksum] of this.lists) {
            estimatedSize += data_view_1.sizeOfASCII(name) + data_view_1.sizeOfASCII(checksum);
        }
    ...
    updateFromDiff({ added, removed }) {
        ...
        if (removed !== undefined && removed.length !== 0) {
            const { networkFilters, cosmeticFilters } = lists_1.parseFilters(removed.join('\n'), this.config);
        ...
        if (added !== undefined && added.length !== 0) {
            const { networkFilters, cosmeticFilters } = lists_1.parseFilters(added.join('\n'), this.config);
    ...
    blockScripts() {
        this.updateFromDiff({
            added: [dsl_1.block().scripts().redirectTo('javascript').toString()],
        });
        return this;
    }
    blockImages() {
        this.updateFromDiff({
            added: [dsl_1.block().images().redirectTo('png').toString()],
        });
        return this;
    }
    blockMedias() {
        this.updateFromDiff({
            added: [dsl_1.block().medias().redirectTo('mp4').toString()],
        });
        return this;
    }
    blockFrames() {
        this.updateFromDiff({
            added: [dsl_1.block().frames().redirectTo('html').toString()],
        });
        return this;
    }
    blockFonts() {
        this.updateFromDiff({
            added: [dsl_1.block().fonts().toString()],
        });
        return this;
    }
    blockStyles() {
        this.updateFromDiff({
            added: [dsl_1.block().styles().toString()],
        });
        return this;
    }
}
