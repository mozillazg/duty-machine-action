...
const captureWebsite = async (input, options) => {
	...
	if (screenshotOptions.fullPage) {
		...
		const bodyBoundingHeight = bodyBoundingBox.height;
		await bodyHandle.dispose();
		const offsetHeight = await page.evaluate(() => document.documentElement.offsetHeight);
		const scrollHeight = await page.evaluate(() => document.documentElement.scrollHeight);
		const maxHeight = max([bodyBoundingHeight, offsetHeight, scrollHeight])
		console.info(`page max height choice: ${bodyBoundingHeight}, ${offsetHeight}, ${scrollHeight}, select ${maxHeight}`)

		// Scroll one viewport at a time, pausing to let content load
		const viewportHeight = viewportOptions.height;
		let viewportIncrement = 0;
		while (viewportIncrement + viewportHeight < maxHeight) {
			const navigationPromise = page.waitForNavigation(
				{timeout: 1000, waitUntil: 'networkidle0'}).catch((e) => {});
			...
			await page.waitForTimeout(1000).catch((e) => {});
		...
		await page.waitForSelector('body', {timeout: timeoutInSeconds}).catch((e) => {console.error(e)});
		// Scroll back to top
		...
		await page.waitForFunction(imagesHaveLoaded, {timeout: timeoutInSeconds}).catch((e) => {console.error(e)});;
		...
		// let maxScaleFactor = Number.parseFloat((maxTextureSize / maxHeight).toFixed(2));
		// const deviceScaleFactor = viewportOptions.deviceScaleFactor
		// console.info(`maxScaleFactor: ${maxScaleFactor}, deviceScaleFactor: ${deviceScaleFactor}`)
		// if (deviceScaleFactor > maxScaleFactor) {
		// 	const fitScaleFactor = deviceScaleFactor * 0.87;
		// 	if (maxScaleFactor < fitScaleFactor) {
		// 		if (maxHeight > viewportOptions.height * 15 && maxHeight < viewportOptions.height * 30) {
		// 			maxScaleFactor = fitScaleFactor;
		// 			console.info(`use ${maxScaleFactor} as maxScaleFactor`);
